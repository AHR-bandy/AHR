...
### メモ（ソフトマックス関数）
<br>   exp(Ai)/Σexp(Ai)
<br>   元の数値が大きい程大きい値を出力する
<br>   恐らくソフトマックスという名前からマックス関数（最大の値のみを出力）を緩やかにした、
<br>   最大ではないが大きい数値も含めて出力することを想定されたもの
<br>   この関数は値が大きすぎると計算不能となってしまう
<br>   exp(Ai)/Σexp(Ai)=Cexp(Ai)/CΣexp(Ai)=exp(Ai+logC)/Σexp(Ai+logC)=exp(Ai+C')/Σexp(Ai+C')
<br>   この時、C'=MAX(Ai)とすれば、当初よりは値の大きさによる問題は解消される（必ず解消されるわけではない）
